...
if (isset($_GET['cod_matricula'])) {
    ...
    $cod_matricula = $conn->real_escape_string($cod_matricula);

    // Preparar a consulta ao banco de dados sem usar bind_param
    $sql = "SELECT *, cod as cod_matricula  FROM imoveis WHERE cod = '$cod_matricula'";  // Observação: o valor é colocado diretamente na string
    ...
    echo json_encode(["message" => "Parâmetro 'cod_matricula' não enviado."]);
}
