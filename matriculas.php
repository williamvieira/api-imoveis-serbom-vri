...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // SQL para inserir os dados na tabela matriculas
    $sql = "INSERT INTO matriculas (matriculasSelecionadas, cod_matricula, apelido, local, numero_relogio, sub_numero_relogio, categoria_consumo, nome_proprietario, cpf_cnpj_proprietario, mesmo_proprietario_matricula, nome_titular_consumidor, cpf_cnpj_titular_consumidor, mesmo_titular_usuario, status, debitos_aberto, valor_debitos_aberto, mes_ultimo_competencia, cadastro_atualizado, observacoes, date_insert, date_update) 
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        // Atualizando o campo 'cod' na tabela 'matriculas'
        $sqlUpdateCodigo = "UPDATE matriculas SET cod='REN$lastID' WHERE id='$lastID'";
        $conn->query($sqlUpdateCodigo);
        
        // Selecionando registros da tabela GED onde o register_id é 'MTnull'
        $sqlSelectGED = "SELECT * FROM GED WHERE register_id = 'RENnull'";
        ...
        if ($resultGED->num_rows > 0) {
            ...
            while ($row = $resultGED->fetch_assoc()) {
                $sqlUpdateGED = "UPDATE GED SET register_id = 'REN$lastID' WHERE id = " . $row['id'];
