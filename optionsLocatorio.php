...
if (isset($_GET['q']) && !empty($_GET['q'])) {
    ...
    $sql = "SELECT cod AS cod_locatorio, apelidoLocatario FROM locatario WHERE LOWER(cod) LIKE '%$query%' OR apelidoLocatario LIKE '%$query%' LIMIT 10";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $imoveis[] = [
                'value' => $row['cod_locatorio'], // Código do imóvel
                'label' => $row['cod_locatorio'] . ' - ' . $row['apelidoLocatario'] // Label para exibição no Select
            ];
        }
    ...
    $sql = "SELECT cod as cod_locatorio, apelidoLocatario FROM locatario LIMIT 10";
    $result = $conn->query($sql);

    $imoveis = [];
    while ($row = $result->fetch_assoc()) {
        $imoveis[] = [
            'value' => $row['cod_locatorio'],
            'label' => $row['cod_locatorio'] . ' - ' . $row['apelidoLocatario']
        ];
    }
